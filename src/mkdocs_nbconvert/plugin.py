...
class NbConvertPlugin(BasePlugin):
    config_scheme = (
        ("input_dir", config_options.OptionallyRequired(default="notebooks")),
        ("recursive", config_options.OptionallyRequired(default=True)),
        ("output_dir", config_options.OptionallyRequired(default="notebooks")),
    )
    ...
    def on_files(self, files, config, **kwargs):
        ...
        # glob match
        nb_paths_iter = iglob(path.join(config_file_dir, input_dir, "**", "*.ipynb"), recursive=self.config["recursive"])
        # Exporter
        ...
        for nb_path in nb_paths_iter:
            ...
            #
            log.debug("nbconvert: markdown export %s => %s", nb_path, md_path)
            # run nbconvert
            with io.open(nb_path, encoding="utf8") as fp:
            ...
            for resource_name, resource_data in resources["outputs"].items():
                ...
                log.debug(
                    "nbconvert: resource output(%dBytes): %s => %s", len(resource_data), resource_name, resource_dest_path
                )
            ...
            log.debug(
                ...
                file_obj.abs_src_path,
                file_obj.abs_dest_path,
                file_obj.url,
            )
